...
const Details = (props) => {
  ...
  // water, steps, food description, food calories, exercise description, and exercise calories
  ...
  const handleSubmitFood = (e) => {
    ...
    setFoodList((prevState) => {
      return [...prevState, newFoodEntry];

      // QUESTION: Why did ...prevState say it was not iterable when it's in an object -- but it's okay in an array??

      // console.log("food list ", foodList);
      // console.log("prev state: ", prevState);
      // console.log("prev food list ", prevState.foodList);

      //return {
      // ...prevState,
      // foodList: [...prevState, newFoodEntry],
      //};
    });

    // TODO: call the update handler via props and pass in the calories
    props.updateFood(foodCal);
  ...
  const handleSubmitExercise = (e) => {
    ...
    setExerciseList((prevState) => {
      return [...prevState, newExerciseEntry];
    });
    ...
    <div className={`details-container ${props.type}-bkg`}>
      ...
      {props.type === "water" && (
        <div>
          ...
          <p className="info">
            In general, you should drink between &frac12; - 1 ounce for every
            pound you weigh. That's 5-10 glasses per day for a 160-lb person.
          </p>
        </div>
      )}
      ...
      {props.type === "steps" && (
        <div>
          ...
          <p className="subheader steps-subheader">
            What's your daily steps goal?
          </p>
        ...
        <div>
          <form id="food-form">
            ...
            <input
              type="text"
              className="wide"
              placeholder="Description of food or beverage"
              maxLength="32"
              value={foodDesc}
              onChange={handleFoodDescChange}
            />
            <input
              type="number"
        ...
        <div>
          <form id="exercise-form">
            ...
            <input
              type="text"
              className="wide"
              placeholder="Description of exercise activity"
              maxLength="32"
              value={exerciseDesc}
              onChange={handleExerciseDescChange}
            />
            <input
              type="number"
              placeholder="Calories"
              min="0"
              value={exerciseCal}
              onChange={handleExerciseCalChange}
            />
            {/* TODO: BONUS! Make the button disabled if either field is empty */}
            <button type="submit" onClick={handleSubmitExercise}>
              Update
            </button>
          </form>
          {/* TODO: BONUS! Make the log & table appear only when the first entry is submitted */}
